...
class Grid(torch.nn.Module):
    def __init__(self,i,j):
        ...
        self.num_states = i*j
        self.actions = 4
        self.criticl1 = nn.Linear(self.num_states, hidden_layer)
        self.criticl2 = nn.Linear(hidden_layer, 1)
        
        self.alayer1 = nn.Linear(self.num_states, hidden_layer)
        self.alayer2 = nn.Linear(hidden_layer, self.actions)
    ...
    def forward(self, state):
        ...
        policy_dis = fn.relu(self.alayer1(state))
        actionval = self.alayer2(policy_dis)
        policy_distribution = fn.softmax(actionval)
...
if __name__ == '__main__':
    ...
    for episode in range(num_episodes):
        ...
        for steps in range(num_step):
            obs = obs.long()
            obs = fn.one_hot(obs, num_classes = 2500)
            value, actionval, policy_distribution = agent.forward(obs)
            value = value.detach().numpy()[0,0]
            pdist = policy_distribution.detach().numpy()
            action = torch.argmax(policy_distribution)
            log_prob = torch.log(policy_distribution.squeeze(0)[action])
            entropy = -np.sum(np.mean(pdist) * np.log(pdist))
            print(action)
            action = actionpossibilities[action]
            ...
            if done or steps == num_step-1:
                Qval, _ = agent.forward(new_state)
